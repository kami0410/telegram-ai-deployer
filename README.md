...
- **模型可选**：默认 DeepSeek V4 Flash（更快更省），可选 V4 Pro；`/ask` 问答固定启用思考模式。
- **生成的机器人能力**：人格系统（导入/自定义人格、`/persona-add`、`/persona-rollback`、版本历史与导出）、长期记忆（D1 + 向量检索 + 冲突确认）、北京时间每天 2–3 次轻量主动联系、每周回顾、提醒与周报，以及 Telegram Web App 管理面板。
...
- **v1.2.1**：优化回复气泡拆分——段落优先作为气泡分界，超限句子从头部合并、段落超限时合并最短相邻段，避免最后一个气泡又长又带换行。
- **v1.2.0**：生成机器人模板同步至最新版：新增记忆图（`memory_graph`）、身份核心（`identity_core`）、记忆召回解释（`memory_recall_traces`）、主动联系决策记录与质量事件统计；管理面板新增“关系”与“召回”视图；`/ask` 固定启用思考模式（移除已失效的思考模式开关）；新增公开状态页与 Telegram 机器人头像展示；增加 `0016`–`0021` 六个 D1 迁移；发布质量评测与发布门禁工具（`evaluate:realism` / `release:gate`）。
- **v1.1.0 正式版**：主动联系改为按北京时间每天随机 2–3 次、相邻至少 4 小时；即使上一次未获回复也可自然换话题继续联系，待处理的用户消息会优先并将主动联系顺延 1 小时。
...
- 当前状态：`v1.2.1` 正式版。软件仍按“无担保”方式提供；重要数据请定期使用生成项目内的加密备份工具保存副本。

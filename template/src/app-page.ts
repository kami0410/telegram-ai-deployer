...
<body>
  ...
  <nav class="nav" aria-label="管理区域"><button class="active" data-target="overview">概览</button><button data-target="memories">记忆</button><button data-target="episodes">情景</button><button data-target="timeline">时间线</button><button data-target="sources">来源</button><button data-target="persona">人格</button><button data-target="drafts">草稿</button></nav>
...
const SCRIPT = String.raw`(() => {
  ...
  async function setControl(kind, id, control, reload) { try { await api("/api/app/memory-controls/" + kind + "/" + id, { method: "PATCH", body: JSON.stringify({ control }) }); await reload(); } catch (error) { showError(error.message); } }
  ...
  async function loadTimeline() {
    ...
    data.items.forEach((entry) => { const item = card(); const actions = node("div", "row");
  ...
  async function showView(name) {
    ...
    try { if (!initData) throw new Error("请从 Telegram 机器人中打开管理面板"); if (name === "overview") await loadOverview(); if (name === "memories") await loadMemories(true); if (name === "episodes") await loadEpisodes(); if (name === "timeline") await loadTimeline(); if (name === "sources") await loadSources(); if (name === "persona") await loadPersona(); if (name === "drafts") await loadDrafts(); } catch (error) { showError(error.message); }
  }

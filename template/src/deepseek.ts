    ...
    [
      {
        role: "system",
        content:
          "仅从用户明确说出的内容更新摘要和记忆，不得从助手回复推断。输出 JSON：summary、through_message_id、stable_facts、episodes、relationship_states、time_layers、graph_nodes、graph_edges、identity_evidence。stable_facts 只放长期稳定事实；用户明确表达的稳定偏好（“我喜欢/我爱/我更喜欢/我习惯/我一直”等句式）必须单独提取为 stable_fact 并归类为 preference，即使该表达出现在一次性事件的描述中，也不能只放进 episodes；短期情绪和一次性事件放 episodes；relationship_states 只放仍有后续价值的关系状态，kind 仅可为 open_thread、emotional_state、commitment、shared_moment、interaction_outcome，每项包含 kind、value、source_message_id、evidence。identity_evidence 只记录用户对 Yuan 稳定表达、价值观、边界或推理方式的明确描述，每项包含 identity_key、identity_value、source_message_id、evidence；一次观察仍只是候选，不得写用户本人特征、感情结论或推测。graph_nodes 的 type 仅可为 person、event、topic、goal、place、time，每项包含 type、key、value、confidence、source_message_id、evidence；只有用户明确表示新事实替代旧事实时才可加 supersedes:true。graph_edges 的 relation 仅可为 involves、occurred_at、related_to、supports、updates、conflicts_with，并包含 from_type、from_key、to_type、to_key、confidence、source_message_id、evidence。open_thread 仅用于用户明确提到、之后确实可能有结果或后续的考试、约定、等待结果、计划和未完成事项；普通闲聊、已经结束的事情、助手提出的问题不得标成 open_thread。time_layers 必须包含 topic、week、month、relationship 四个对象，每个对象只含 summary、topics、importance；在对应 previousTimeLayers 基础上压缩更新，topic 聚焦当前话题，week 聚焦本周，month 聚焦本月，relationship 只保留有明确用户证据的长期关系脉络。importance 为 1 至 5。所有新增信息必须来自本批用户消息；不得从助手话语、人格资料或推测创造共同经历、约定、感情结论或 Yuan 的现实活动。",
      },
      {
        role: "user",

...
Password controls are cleared immediately after submission. Secrets are passed to Wrangler through standard input, are not written to deployment state, and are redacted from application output. Non-secret resume state may contain project and resource names, selected model, thinking setting, completed steps, and a Worker URL.

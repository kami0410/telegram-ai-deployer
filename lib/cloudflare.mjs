...
export function resourceNames(projectName) {
  return {
    ...
    imageCache: projectName + "-image-cache",
  };
}

export function isWranglerAuthenticated(output) {
  const text = String(output);
  return /logged in/iu.test(text) && !/not authenticated/iu.test(text);
}

export function isWorkersDevSubdomainError(detail) {
  return /register a workers\.dev subdomain|workers\.dev subdomain/iu.test(String(detail));
}

export function healthCheckFailureMessage(workerUrl, lastError) {
  return `Worker 已部署，但健康检查暂时失败（${lastError}）。请稍后点击恢复部署，或手动打开 ${workerUrl}/health 检查。`;
}
...
export function createDeploymentDependencies({
  ...
  const trustedEnvironment = { ...runtimeEnvironment, ELECTRON_RUN_AS_NODE: "1" };

  async function checked(executable, args, cwd, input) {
    const result = await runCommand(executable, args, { cwd, input, redact: redactor, onOutput, env: trustedEnvironment });
  ...
  async function writeConfig(input, state, workerUrl = "https://example.invalid") {
    ...
    const imageCacheId = state.resources.imageCacheId;
    if (typeof databaseId !== "string") throw new Error("D1 database ID is unavailable");
    if (typeof imageCacheId !== "string") throw new Error("KV namespace ID is unavailable");
  ...
  async function runStep(step, { input, state }) {
    ...
    if (step === "environment") {
      const whoami = await wrangler(["whoami"], appRoot);
      if (!isWranglerAuthenticated(whoami)) {
        throw new Error("Cloudflare 未登录：请返回首页重新执行环境检查完成授权，或为本应用设置 CLOUDFLARE_API_TOKEN 环境变量后重试");
      }
      return {};
    }
    if (step === "template") {
    ...
    if (step === "kv") {
      const listText = await wrangler(["kv", "namespace", "list"], input.outputDir);
      const existing = parseJsonOutput(listText).find((item) => item.title === names.imageCache);
      const title = names.imageCache;
      if (!existing) {
        const createdText = await wrangler(["kv", "namespace", "create", title], input.outputDir);
        const created = parseJsonOutput(createdText).find((item) => typeof item.id === "string");
        if (!created) throw new Error("Unable to resolve KV namespace ID");
        return { resources: { imageCacheId: created.id } };
      }
      return { resources: { imageCacheId: existing.id } };
    }
    if (step === "vectorize") {
    ...
    if (step === "first-deploy") {
      ...
      let output;
      try {
        output = await wrangler(["deploy", "--minify"], input.outputDir);
      } catch (error) {
        const detail = error instanceof Error ? error.message : String(error);
        if (isWorkersDevSubdomainError(detail)) {
          throw new Error("Cloudflare 首次部署需要先注册 workers.dev 子域名。请打开 Cloudflare Dashboard 的 Workers & Pages → Overview，完成注册后点击恢复部署。注册一次后以后项目无需重复操作。");
        }
        throw error;
      }
    ...
    if (step === "secrets") {
      let telegram;
      try {
        telegram = await fetcher("https://api.telegram.org/bot" + input.telegramToken + "/getMe").then((response) => response.json());
      } catch {
        throw new Error("无法连接 api.telegram.org，请检查网络或代理后重试");
      }
      if (!telegram.ok) throw new Error("Telegram token 无效，请检查后重新输入");
      let deepseek;
      try {
        deepseek = await fetcher("https://api.deepseek.com/models", {
          headers: { authorization: "Bearer " + input.deepseekKey },
        });
      } catch {
        throw new Error("无法连接 api.deepseek.com，请检查网络或代理后重试");
      }
      if (!deepseek.ok) throw new Error("DeepSeek key 无效（HTTP " + deepseek.status + "），请检查后重新输入");
    ...
    if (step === "health") {
      ...
      throw new Error(healthCheckFailureMessage(state.workerUrl, lastError));
    }

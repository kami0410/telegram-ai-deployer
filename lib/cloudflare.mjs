...
export function createDeploymentDependencies({
  ...
  async function checked(executable, args, cwd, input) {
    const result = await runCommand(executable, args, { cwd, input, redact: redactor, onOutput });
  ...
  async function runStep(step, { input, state }) {
    ...
    if (step === "environment") {
      await checked(npx, ["wrangler", "whoami"], appRoot);
      return {};
    }
    if (step === "template") {
      const personaText = await readPersona(input);
      await generateProject({ templateRoot, outputDir: input.outputDir, personaText });
      await checked(npm, ["install"], input.outputDir);
      return {};
    }
    if (step === "d1") {
      let databases = parseJsonOutput(await checked(npx, ["wrangler", "d1", "list", "--json"], input.outputDir));
      let database = databases.find((item) => item.name === names.database);
      if (!database) {
        await checked(npx, ["wrangler", "d1", "create", names.database], input.outputDir);
        databases = parseJsonOutput(await checked(npx, ["wrangler", "d1", "list", "--json"], input.outputDir));
    ...
    if (step === "queues") {
      const list = await checked(npx, ["wrangler", "queues", "list"], input.outputDir);
      if (!list.includes(names.queue)) await checked(npx, ["wrangler", "queues", "create", names.queue], input.outputDir);
      if (!list.includes(names.deadLetterQueue)) {
        await checked(npx, ["wrangler", "queues", "create", names.deadLetterQueue], input.outputDir);
      }
      return { resources: { queue: names.queue, deadLetterQueue: names.deadLetterQueue } };
    }
    if (step === "vectorize") {
      const indexes = parseJsonOutput(await checked(npx, ["wrangler", "vectorize", "list", "--json"], input.outputDir));
      if (!indexes.some((item) => item.name === names.vectorize)) {
        await checked(npx, ["wrangler", "vectorize", "create", names.vectorize, "--dimensions=1024", "--metric=cosine"], input.outputDir);
      }
      const metadataText = await checked(npx, ["wrangler", "vectorize", "list-metadata-index", names.vectorize, "--json"], input.outputDir);
      for (const [property, type] of [["owner_id", "number"], ["kind", "string"], ["category", "string"], ["occurred_at", "number"], ["active", "boolean"]]) {
        if (!metadataText.includes('"' + property + '"')) {
          await checked(npx, ["wrangler", "vectorize", "create-metadata-index", names.vectorize, "--propertyName=" + property, "--type=" + type], input.outputDir);
        }
      }
    ...
    if (step === "migration") {
      ...
      await checked(npx, ["wrangler", "d1", "migrations", "apply", names.database, "--remote"], input.outputDir);
      return {};
    }
    if (step === "first-deploy") {
      await writeConfig(input, state);
      const output = await checked(npx, ["wrangler", "deploy", "--minify"], input.outputDir);
    ...
    if (step === "secrets") {
      ...
      await checked(npx, ["wrangler", "secret", "bulk"], input.outputDir, JSON.stringify({
